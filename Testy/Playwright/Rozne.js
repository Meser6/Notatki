//Playwright Test for VSCode - bardzo przydatne rozszerzenie do VSC

// ---------- komendy w terminalu ----------
{
  `npm init playwright@latest` // inicjalizacja projektu w playwright
  `npx playwright show--report` // odpali lokalny serwer na ktorym bedzie widoczny ostatni raport
  `npx playwright test` // odpalenie wszystkich testów w trybie headless na wszystkich projektach zdefiniowanych w konfigu
  `npx playwright test --project=chromium` //odpali odpowiedni projekt
  `npx playwright test --headed` // odpalenie ale z widoczna przegladarka (wlaczy se ona na czas testow i wylaczy po zakonczeniu)
...
test.describe("testy", () => {
  ...
  test.skip("", async () => {}); // zignoruje test
  test.only("", async () => {}); // jesli jest chociaz jedno only to odali tylko te z ta flaga
});
